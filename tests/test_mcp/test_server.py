...
from brainplorp.mcp.server import (
    _plorp_start_day,
...
async def test_plorp_start_day():
    ...
    with patch("brainplorp.mcp.server.start_day") as mock_start:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
...
async def test_plorp_get_review_tasks():
    ...
    with patch("brainplorp.mcp.server.get_review_tasks") as mock_review:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
...
async def test_plorp_add_review_notes():
    ...
    with patch("brainplorp.mcp.server.add_review_notes") as mock_add:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
...
async def test_plorp_mark_task_completed():
    ...
    with patch("brainplorp.mcp.server.mark_completed") as mock_mark:
...
async def test_plorp_defer_task():
    ...
    with patch("brainplorp.mcp.server.defer_task") as mock_defer:
...
async def test_plorp_drop_task():
    ...
    with patch("brainplorp.mcp.server.drop_task") as mock_drop:
...
async def test_plorp_set_task_priority():
    ...
    with patch("brainplorp.mcp.server.set_priority") as mock_priority:
...
async def test_plorp_get_inbox_items():
    ...
    with patch("brainplorp.mcp.server.get_inbox_items") as mock_inbox:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
...
async def test_plorp_create_task_from_inbox():
    ...
    with patch("brainplorp.mcp.server.create_task_from_inbox") as mock_create:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
...
async def test_plorp_create_note_from_inbox():
    ...
    with patch("brainplorp.mcp.server.create_note_from_inbox") as mock_create:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
            mock_create.return_value = {
...
async def test_plorp_create_both_from_inbox():
    ...
    with patch("brainplorp.mcp.server.create_both_from_inbox") as mock_create:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
            mock_create.return_value = {
...
async def test_plorp_discard_inbox_item():
    ...
    with patch("brainplorp.mcp.server.discard_inbox_item") as mock_discard:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
...
async def test_plorp_create_note():
    ...
    with patch("brainplorp.mcp.server.create_note_standalone") as mock_create:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
            mock_create.return_value = {
...
async def test_plorp_create_note_with_task():
    ...
    with patch("brainplorp.mcp.server.create_note_linked_to_task") as mock_create:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
            mock_create.return_value = {
                "note_path": "/vault/notes/test.md",
...
async def test_plorp_link_note_to_task():
    ...
    with patch("brainplorp.mcp.server.link_note_to_task") as mock_link:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
...
async def test_plorp_get_task_info():
    ...
    with patch("brainplorp.mcp.server.tw_get_task_info") as mock_get:
...
async def test_plorp_process_daily_note_step1():
    ...
    with patch("brainplorp.mcp.server.process_daily_note_step1") as mock_step1:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
...
async def test_plorp_process_daily_note_step2():
    ...
    with patch("brainplorp.mcp.server.process_daily_note_step2") as mock_step2:
        with patch("brainplorp.mcp.server._get_vault_path") as mock_vault:
            mock_vault.return_value = Path("/vault")
